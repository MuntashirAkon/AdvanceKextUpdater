...
+(NSArray<pciDevice *> *)readIDs{
    FILE *handle = fopen([[[KextHandler.pciIDsCachePath stringByAppendingPathComponent:@"pci"] stringByAppendingPathExtension:@"ids"] fileSystemRepresentation], "rb");
...
+ (BOOL) checkForDBUpdate {
    @try {
        ...
        NSString *path = KextHandler.pciIDsCachePath;
        if(![NSFileManager.defaultManager fileExistsAtPath:path]) {
            ...
            task.arguments = @[@"-C", path, @"pull", @"origin", @"master"];
            [task launch];
            [task waitUntilExit];
...
+ (BOOL) initDB {
    ...
    @try {
        NSTask *task = [[NSTask alloc] init];
        task.launchPath = git_exec;
        task.arguments = @[@"clone", PCIID_REPO, KextHandler.pciIDsCachePath];
        [task launch];
        [task waitUntilExit];
        return task.terminationStatus == 0 ? YES : NO;
    } @catch(NSError *e) {}
